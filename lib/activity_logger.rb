module ActivityLogger
  ...
  def add_activities(options = {})
    ...
    people_ids = people_to_add(user, activity, include_person)
  ...
  private
    ...
    def people_to_add(user, activity, include_person)
      all = user.friends_of.map(&:id)
      all.push(user.id) if include_person
    ...
    def already_have_activity(users, activity)
      Feed.all(:select => "DISTINCT user_id",
